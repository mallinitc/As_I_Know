    ...
    Foreach($Cluster in $Clusters)
    {
        ...
        ForEach($RDGHost in $RDGateways)
        {
            ...
            else
            {
                ...
                if($avlRDGsCount -lt 2)
                {
                    #only 1 Enabled Server
                    Write-Host "[ERR]   $($org.name)/$($Cluster.name)/$($RDGHost.name) : Can't change the mode because there is only $($avlRDGsCount) Enabled servers." -ForegroundColor Red
                    break
                }
                else
                {
            ...
            $RDGNow = Get-RDGHost -id $org.id -clus_id $Cluster.clusterId -reg_id $Cluster.Clus_RegionId -host_id $RDGHost.id
            If($RDGNow.mode -notlike 'Maintenance')
            {
                Write-Host "[ERR]   $($org.name)/$($Cluster.name)/$($RDGHost.name) : is not in Maintenance mode, so breaking" -ForegroundColor Red
                break
            }
            else
            {
                Write-Host "[INFO]  $($org.name)/$($Cluster.name)/$($RDGHost.name): is in Maintenance mode." -ForegroundColor Green
                #Take Snapshot
