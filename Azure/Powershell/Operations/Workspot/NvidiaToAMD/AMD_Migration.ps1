...
#Import-Csv -Path C:\H_Skus\AzureCSP-USA_Customer_List.csv|Out-GridView -PassThru
...
foreach($obj in $Jobobj)
{
    ...
    If ($Output.Value[0].Message -like '*Failed*')
    {
        $Job=Set-AzVM  -Name $vmname -ResourceGroupName $vm.ResourceGroupName -Redeploy -AsJob
        $RedeployVMsObj.Add($Vmname)
    }
    else
    {
...
#Rechecking all the redeployed VMs
[System.Collections.ArrayList] $Jobobj = @()
Foreach( $VmName in $RedeployVMsObj )
{
    $Vm =  $AllAzVMs|Where-Object{$_.Name -like $Vmname}
    Sleep -Seconds 10
    $Job=Invoke-AzVMRunCommand  -ResourceGroupName $Vm.ResourceGroupName -VMName $Vmname -CommandId RunPowerShellScript -ScriptPath $Path\AMD_GPU_PostDeployement.ps1 -AsJob
...
{
    $Output = Receive-Job -Job $obj
    $OutputMsg=$Output.Value[0].Message
    $VmName=$OutputMsg.Split("+")[0]
    $Status=$OutputMsg.Split("+")[1]
    $VmName = $VmName.Trim()
    $Vm =  $AllAzVMs | Where-Object{$_.Name -like $vmname}
    $NewAzVm = Get-AzVM -Name $Vmname -ResourceGroupName $vm.ResourceGroupName
    $UpdatedSize= ($NewAzVm).HardwareProfile.VmSize
